...
public abstract class FeautureIdeUtils {
	public static void createPackageForClasses(Asset e) {
		if (e.getType().equals("classe")) {
			...
			if(!Arrays.asList(new File(".").listFiles()).contains(new File(".\\"+e.getParent().getValueByRole(CtRole.NAME)+""))) {
				...
				new File(".\\cluster\\"+(e.getParent().getValueByRole(CtRole.NAME).toString().equals("unnamed package")?"source":(e.getParent().getValueByRole(CtRole.NAME)+""))).mkdirs();
			}
		}
	};
}
